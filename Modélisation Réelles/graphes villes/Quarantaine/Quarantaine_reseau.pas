...
const Xmax           = 11;
	  ville_max      = 550;				// nombre max de ville
...
for i := 1 to villes do 
	begin
	beta[i].A := 2  * R0*(0.5+((ln(densite[i])-ln(dmin))/(ln(dmax)-ln(dmin)))) / LambdaB;
	beta[i].B := 1  * R0*(0.5+((ln(densite[i])-ln(dmin))/(ln(dmax)-ln(dmin)))) / LambdaB;
	beta[i].G := 0.5* R0*(0.5+((ln(densite[i])-ln(dmin))/(ln(dmax)-ln(dmin)))) / LambdaG;
	end;
close(fichier);
